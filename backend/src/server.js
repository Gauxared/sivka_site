...
function sendMediaFile(req, res, asset) {
  const dataUrl = String(asset?.url || '');
  const match = dataUrl.match(/^data:([^;,]+)(;base64)?,(.*)$/);
  if (!match) return fail(req, res, 404, 'MEDIA_FILE_NOT_FOUND', 'Файл медиатеки не найден');

  const mimeType = asset.mimeType || match[1] || 'application/octet-stream';
...
function getRequestSessionTokens(req) {
  const bearer = String(req.headers.authorization || '').match(/^Bearer\s+(.+)$/i)?.[1];
  const cookieSid = parseCookies(req).sid;
  return { bearer, cookieSid };
}

function getSession(req) {
  const { bearer, cookieSid } = getRequestSessionTokens(req);
  if (bearer) {
    const bearerUser = getSessionUser(bearer);
    if (bearerUser) return bearerUser;
  }
  if (cookieSid && cookieSid !== bearer) return getSessionUser(cookieSid);
  return undefined;
}

function setSessionCookie(res, sid) {
  res.setHeader('Set-Cookie', `sid=${encodeURIComponent(sid)}; HttpOnly; Path=/; Max-Age=${SESSION_COOKIE_MAX_AGE_SECONDS}; SameSite=Lax`);
}
...
function getMediaFingerprint(asset) {
  ...
  if (!fileName || sizeBytes <= 0) return '';

  return `file:${fileName}:${sizeBytes}:${width}:${height}`;
}

function findDuplicateMediaAsset(mediaAssets, body) {
  const fingerprint = getMediaFingerprint(body);
  if (!fingerprint) return undefined;
  return mediaAssets.find((asset) => getMediaFingerprint(asset) === fingerprint);
}
...
function buildAttentionBookings(db) {
  ...
  return db.bookings
    .map((booking) => {
      ...
      if (!booking.assignedTrainerId) reasons.push('Тренер не назначен');
      if (booking.assignedHorses.length < booking.participants.length) reasons.push('Не всем участникам назначены лошади');
      if (booking.assignedHorses.some((pair) => !activeHorseIds.has(pair.horseId))) reasons.push('Назначена недоступная лошадь');
      if (booking.assignedHorses.some((pair) => (restWarnings.get(pair.horseId) || 0) > 0)) reasons.push('Возможен конфликт отдыха лошади');
...
function canAssignTrainer(db, booking, trainer) {
  ...
  if (!trainer.isActive || trainer.status !== 'active') return 'Тренер недоступен';
  if (!trainer.allowedServiceIds.includes(booking.serviceId)) return 'Тренер не проводит выбранную услугу';
  ...
  if (toMinutes(booking.startTime) < toMinutes(trainer.workStartTime) || toMinutes(booking.endTime) > toMinutes(trainer.workEndTime)) {
    return 'Тренер недоступен в выбранное время';
  }
  ...
  return conflict ? 'У тренера уже есть занятие в выбранное время' : '';
}
...
async function handleAuth(req, res, method, parts, body, db) {
  if (method === 'POST' && parts[1] === 'login') {
    ...
    return ok(req, res, { ...publicUser(user), sessionToken: session.token });
  }

  if (method === 'POST' && parts[1] === 'logout') {
    const { bearer, cookieSid } = getRequestSessionTokens(req);
    if (bearer) deleteSession(bearer);
    if (cookieSid && cookieSid !== bearer) deleteSession(cookieSid);
...
function isPublicRequest(method, parts, pathname) {
  ...
  if (method === 'GET' && parts[0] === 'media' && parts[2] === 'file') return true;
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (parts[0] === 'availability' && parts[1] === 'dates' && method === 'GET') {
      ...
      const days = Number(query.get('days') || 120);
      return ok(req, res, getAvailableDates(db, serviceId, participants, days));
    }
    ...
    if (parts[0] === 'bookings' && method === 'POST' && parts.length === 1) {
      if (!body.personalDataAgreement) return fail(req, res, 400, 'PERSONAL_DATA_REQUIRED', 'Нужно согласие на обработку персональных данных');
      const availability = checkBookingAvailability(db, body);
      if (!availability.isAvailable) return fail(req, res, 409, 'SLOT_UNAVAILABLE', availability.reasons[0] || 'Выбранный слот недоступен', availability);
      ...
      saveDb();
      return created(req, res, { requestId: booking.id }, 'Заявка создана');
    }

    if (parts[0] === 'bookings' && parts[1]) {
      const booking = db.bookings.find((item) => item.id === parts[1]);
      if (!booking) return fail(req, res, 404, 'BOOKING_NOT_FOUND', 'Заявка не найдена');
      if (req.user?.role === 'trainer' && booking.assignedTrainerId !== req.user.trainerId) {
        return fail(req, res, 403, 'FORBIDDEN_BOOKING', 'Тренер может открыть только назначенные ему заявки');
      }
      ...
      if (method === 'PATCH' && parts[2] === 'assign-trainer') {
        if (body.trainerId) {
          ...
          if (!trainer) return fail(req, res, 404, 'TRAINER_NOT_FOUND', 'Тренер не найден');
      ...
      if (method === 'PATCH' && parts[2] === 'trainer-status') {
        if (req.user?.role === 'trainer' && booking.assignedTrainerId !== req.user.trainerId) {
          return fail(req, res, 403, 'FORBIDDEN_BOOKING', 'Тренер может менять только назначенные ему заявки');
        }
        ...
        createNotification(db, {
          ...
          channel: 'in_app',
          title: 'Тренер обновил статус',
          message: `Статус тренера по заявке на ${booking.date} изменен: ${body.trainerStatus}.`,
          bookingId: booking.id,
        });
        createNotification(db, {
          recipientRole: 'manager',
    ...
    if (parts[0] === 'media') {
      ...
      if (method === 'GET' && parts[2] === 'file') {
        return sendMediaFile(req, res, db.mediaAssets.find((asset) => asset.id === parts[1]));
      }
      if (method === 'POST' && parts.length === 1) {
        const duplicateAsset = findDuplicateMediaAsset(db.mediaAssets || [], body);
        if (duplicateAsset) return ok(req, res, duplicateAsset, 'Изображение уже есть в медиатеке');
        return created(req, res, createCollectionItem(db, 'mediaAssets', 'media', { ...body, createdAt: new Date().toISOString() }));
      }
