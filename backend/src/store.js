...
const DATA_FIX_VERSION = '2026-06-07-media-references';
...
function writeStaffAccounts(accounts, trainers) {
  ...
  try {
    ...
    const obsoleteUserIds = existingRows.map((row) => row.id).filter((id) => !normalizedIds.has(id));
    if (obsoleteUserIds.length > 0) {
      const deleteObsolete = db.prepare('DELETE FROM staff_users WHERE id = ?');
      obsoleteUserIds.forEach((id) => deleteObsolete.run(id));
    }
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
...
function replaceKnownText(value) {
  if (typeof value === 'string') {
    return value
      .replaceAll('г. Гурьевск', 'пгт. Шерегеш')
      .replaceAll('Гурьевск', 'Шерегеш')
      .replaceAll('ИП Орлова Н.И.', 'КТК "Сивка-Бурка"')
      .replaceAll('Selected date is closed', 'Выбранный день закрыт для записи')
      .replaceAll('Horse is unavailable for selected time', 'Лошадь недоступна в выбранное время')
      .replaceAll('No free horses for selected time', 'Нет свободных лошадей для выбранного времени')
      .replaceAll('Horse needs rest after previous lesson', 'Лошадь должна отдохнуть после предыдущего занятия')
      .replaceAll('No free trainer for selected time', 'Нет свободного тренера для выбранного времени')
      .replaceAll('Trainer does not provide selected service', 'Тренер не проводит выбранную услугу')
      .replaceAll('Trainer does not work on selected date', 'Тренер не работает в выбранный день')
      .replaceAll('Trainer is unavailable for selected time', 'Тренер недоступен в выбранное время')
      .replaceAll('Trainer is unavailable', 'Тренер недоступен')
      .replaceAll('Service is temporarily unavailable', 'Услуга временно недоступна')
      .replaceAll('Personal data agreement is required', 'Нужно согласие на обработку персональных данных')
      .replaceAll('Selected slot is unavailable', 'Выбранный слот недоступен');
  }

  if (Array.isArray(value)) return value.map(replaceKnownText);
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, replaceKnownText(item)]));
  }
  return value;
}
...
function createMediaAssetFromDataUrl(dataUrl, title) {
  const hash = crypto.createHash('sha1').update(dataUrl).digest('hex').slice(0, 16);
  const mimeType = dataUrl.match(/^data:([^;,]+)/)?.[1] || 'image/jpeg';
  const extension = mimeType.includes('png') ? 'png' : mimeType.includes('webp') ? 'webp' : 'jpg';
  return {
    id: `media-migrated-${hash}`,
    fileName: `${title || 'image'}-${hash}.${extension}`,
    title: title || 'Изображение',
    altText: title || 'Изображение',
    url: dataUrl,
    mimeType,
...
function extractEmbeddedMedia(value, mediaByUrl, title = 'Изображение', key = '') {
  if (isImageDataUrl(value) && ['image', 'photo', 'homeHeroImage'].includes(key)) {
    let asset = mediaByUrl.get(value);
    if (!asset) {
      asset = createMediaAssetFromDataUrl(value, title);
      mediaByUrl.set(value, asset);
    }
    return `media:${asset.id}`;
  }

  if (Array.isArray(value)) {
    return value.map((item) => extractEmbeddedMedia(item, mediaByUrl, title));
  }

  if (value && typeof value === 'object') {
    const nextTitle = typeof value.title === 'string'
      ? value.title
    ...
    return Object.fromEntries(
      Object.entries(value).map(([entryKey, item]) => [
        entryKey,
        extractEmbeddedMedia(item, mediaByUrl, nextTitle, entryKey),
      ]),
    );
  }
...
function applyDataFixesIfNeeded() {
  if (getMeta('data_fix_version') === DATA_FIX_VERSION) return;

  const seed = loadFrontendSeed();
  const currentMediaAssets = readEntityCollection(ENTITY_TABLES.mediaAssets);
  const mediaByUrl = new Map(currentMediaAssets.filter((asset) => isImageDataUrl(asset.url)).map((asset) => [asset.url, asset]));

  SETTINGS_KEYS.forEach((key) => {
    const current = readSetting(key, seed[key]);
    const fixed = extractEmbeddedMedia(replaceKnownText(current), mediaByUrl, key);
    if (JSON.stringify(current) !== JSON.stringify(fixed)) writeSetting(key, fixed);
  });

  Object.entries(ENTITY_TABLES).forEach(([key, table]) => {
    if (key === 'mediaAssets') return;

    const current = readEntityCollection(table);
    let fixed = extractEmbeddedMedia(replaceKnownText(current), mediaByUrl, key);
    if (key === 'bookings' && fixed.length === 0 && Array.isArray(seed.bookings) && seed.bookings.length > 0) {
      fixed = clone(seed.bookings);
    }
    if (JSON.stringify(current) !== JSON.stringify(fixed)) replaceEntityCollection(table, fixed);
  });

  const currentMediaAssetIds = new Set(currentMediaAssets.map((asset) => asset.id));
  const migratedMediaAssets = Array.from(mediaByUrl.values()).filter((asset) => !currentMediaAssetIds.has(asset.id));
  const nextMediaAssets = [...currentMediaAssets, ...migratedMediaAssets];
  if (JSON.stringify(currentMediaAssets) !== JSON.stringify(nextMediaAssets)) {
    replaceEntityCollection(ENTITY_TABLES.mediaAssets, nextMediaAssets);
  }

  setMeta('data_fix_version', DATA_FIX_VERSION);
}
